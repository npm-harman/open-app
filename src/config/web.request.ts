...
export class WebRequest implements IWebRequest {
  ...
  patch: <T>(url: string, data?: any, header?: any, goToErrorState?: boolean) => Observable<HttpEvent<T>>;
  ...
  constructor(private http: HttpClient, @Inject('ICommonService') private commonService: ICommonService) {
    ...
    vm.patch = <T>(url: string, data?: any, header?: any, goToErrorState?: boolean) => {
      const options: any = vm.getRequestOption('patch', url, {}, header ? header : vm.commonService.getRequestHeader());
      return vm.http.patch<T>(url, data, options);
    };
